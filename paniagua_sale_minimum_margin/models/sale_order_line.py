...
class SaleOrderLine(models.Model):
    ...
    @api.constrains('price_unit', 'discount', 'product_id', 'margin_percent')
    def _check_minimum_margin(self):
        ...
        for line in self:
            if line.product_id and line.product_id.minimum_margin > 0:
                # Usar el margin_percent que ya calcula sale_margin
                ...
                if current_margin < line.product_id.minimum_margin:
                    raise ValidationError(
                        ...
                        f"Precio venta: {line.price_unit:.2f} €\n"
                        f"Descuento: {line.discount:.2f} %\n"
                        f"Coste: {line.purchase_price:.2f} €"
                    )
